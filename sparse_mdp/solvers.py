...
class ILESolver:
	 ...
	 def step(self, terminal, transition=None, reward=None, Q0=None, policy=None):
			"""
			"""
...
#         Q0 = np.hstack([t.multiply(r).sum(axis=1) for t,r in zip(transition, reward)])
	 ...
	 def solve(self, policy, update=True):
			"""
			"""
			...
			while not done:
				 V, Q = self.step(terminal, transition, reward, Q0, policy)
